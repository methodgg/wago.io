...
(function (root, name, factory) {
  ...
  function expect(value) {
    ...
    else if (token.type==32 && token.value==")" && wago.anonFunction) { next(); }
  ...
  function parseIdentifier() {
    ...
    if (Identifier !== token.type && token.type==32 && token.value=="(") { wago.anonFunction=true }
    ...
    if (!consume(')')) {
      while (true) {
        if (Identifier === token.type) {
          ...
          expect(')');
          break;
        } else if (!wago.anonFunction) {
          raiseUnexpectedToken('<name> or \'...\'', token);
        }
      }
    }
    wago.anonFunction=false
