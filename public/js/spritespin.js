...
(function($) {
  ...
  var modEvents = [ 'mousedown', 'touchstart']
  ...
  Spin.defaults = {
    ...
    frameTime         : 1000/24,      // Time in ms between updates. (1000 / fps)
...
(function ($) {
  ...
  SpriteSpin.extendApi({
    // added by mark for wago
    setFPS: function(fps){
        this.data.frameTime = 1000/fps
        window.clearInterval(this.data.animation);
        this.data.animation = window.setInterval(this.data.frameFunction, this.data.frameTime);
