<template>
    <div id="edit-macro">  
      ...
      <div class="flex-container">
        <div class="flex-col flex-right" v-if="!this.readonly">
          <md-button v-if="editorLang === 'lua'" @click="formatCode"><md-icon>code</md-icon> {{ $t("Format Lua") }}</md-button>
      ...
      <monaco-editor v-model="luaCode" :lang="editorLang" :readonly="this.readonly"></monaco-editor>
  ...
  export default {
    name: 'edit-macro',
    props: ['cipherKey', 'readonly'],
    data: function () {
      return {
        luaCode: this.$store.state.wago.code.encoded,
        editorFile: '',
        editorLang: 'wowMacro',
        showExport: false,
  ...
  <style>
  #edit-macro .md-select-content { max-height: calc(70vh); margin-bottom: 32px }
  #edit-macro .md-select { width: auto }
  #edit-macro .md-input-container { margin-bottom: 10px}
  #edit-macro .md-input-container:after { content: none }
  #edit-macro .flex-container { display: flex; flex-direction: row; flex-wrap: nowrap; align-items: flex-end}
  #edit-macro .flex-left { order: 0; flex: 0 1 auto}
  #edit-macro .flex-left .md-input-container label { white-space: nowrap}
  #edit-macro .flex-right { order: 1; flex: 1 1 auto; text-align: right}
  #edit-macro .ace_editor { box-shadow: 0 1px 5px rgba(0, 0, 0, 0.2), 0 2px 2px rgba(0, 0, 0, 0.14), 0 3px 1px -2px rgba(0, 0, 0, 0.12); }
