<template>
  <div id="search-weakaura">
    <h2 id="addon-name">WeakAuras - <span>The Burning Crusade</span></h2>
    <md-layout>
    ...
    <md-layout>
      ...
      <md-layout>
        ...
        <md-list class="md-double-line md-dense">
          <md-list-item v-for="raid in tbcRaids" v-bind:key="raid.id" :class="raid.cls + ' md-inset'">
      ...
      <md-layout>
        ...
        <md-list class="md-double-line md-dense">
          <md-list-item v-for="raid in tbcRaids2" v-bind:key="raid.id" :class="raid.cls + ' md-inset'">
            <category-image :group="raid.cls"></category-image>
            <div class="md-list-text-container">
              <a href="#" @click.prevent="searchRoute(raid.slug)">{{ raid.text }}</a>
              <span>
...
export default {
  ...
  methods: {
    ...
    searchRoute: function (slug) {
      this.$store.commit('userSearchOption', {field: 'expansion', value: 'tbc'})
      this.$router.push('/tbc-weakauras/' + slug)
    }
  },
  data: function () {
    return {
  ...
  computed: {
    ...
    // tbcRaids: function () {
    //   return categories.raidCategories(['raidssc', 'raidtk', 'raidmthyjal', 'raidbt', 'raidsw'], this.$t)
    // },
    // tbcRaids2: function () {
    //   return categories.raidCategories(['raidkarazhan', 'raidgruul', 'tbcdungeon'], this.$t)
    // },
    tbcRaids: function () {
      return categories.raidCategories(['raidkarazhan', 'raidgruul', 'raidssc', 'raidtk'], this.$t)
    },
    tbcRaids2: function () {
      return categories.raidCategories(['raidmthyjal', 'raidbt', 'raidsw', 'tbcdungeon'], this.$t)
    },
    misc: function () {
      return categories.getCategories(['gen1', 'gen7', 'gen2', 'gen6', 'gen9', 'gen10', 'gen11', 'gen12', 'gen14', 'gen13', 'gen8', 'gen15', 'gen16', 'gen17'], this.$t, null, 'TBC-WEAKAURA')
    },
    roles: function () {
      return categories.getCategories(['role4', 'role1', 'role2', 'role3'], this.$t)
    },
    pvp: function () {
      return categories.getCategories([/^(arena|bg|wpvp1?)$/], this.$t, null, 'TBC-WEAKAURA')
    },
    mechanics: function () {
      return categories.getCategories([/^mech[\d]+/], this.$t, null, 'TBC-WEAKAURA')
    },
    equip: function () {
      return categories.getCategories([/^equip[\d]+/, 'legen'], this.$t, null, 'TBC-WEAKAURA')
    },
    professions: function () {
      return categories.professionCategories(this.$t, 'TBC-WEAKAURA', 'tbc')
    },
