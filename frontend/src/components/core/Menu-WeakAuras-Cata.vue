    ...
    <div id="search-weakaura">
      ...
      <md-layout>
        ...
        <md-layout>
          ...
          <md-list class="md-double-line md-dense">
            <md-list-item v-for="raid in cataclysmRaids2" v-bind:key="raid.id" :class="raid.id + ' md-inset'">
              <div class="menu-image"></div>
              <div class="md-list-text-container">
                <a :href="'/cataclysm-weakauras/' + raid.slug" @click.prevent="searchRoute(raid)">{{ raid.text }}</a>
                <span>
  ...
  export default {
    ...
    computed: {
      ...
      cataclysmRaids: function () {
        return window.Categories.raidCategories(['raidblackwingdescent', 'raidbastiontwilight', 'raidfourwinds', 'raidfirelands','raidbaradinhold'])
      },
      cataclysmRaids2: function () {
        return window.Categories.raidCategories(['raiddragonsoul', 'catadungeon'])
      },
      misc: function () {
