<template>
  <div id="search-mdt">
    ...
    <md-layout>
      <md-layout md-column>
        <md-subheader>{{ $t("Dragonflight Dungeons") }}</md-subheader>
        <md-list class="md-double-line md-dense">
          <md-list-item v-for="dungeon in dungeons" v-bind:key="dungeon.id" :class="dungeon.id + ' md-inset'">
            <div class="menu-image"></div>
            <div class="md-list-text-container">
              <span>
                <a :href="'/dragonflight-mdt/' + dungeon.slug" @click.prevent="searchRoute(dungeon)" class="subheader">{{ dungeon.text }}</a>
                <template v-for="boss in dungeon.bosses">
                  <a v-if="boss.subheader" v-bind:key="'header'+boss.id" :href="'/dragonflight-mdt/' + dungeon.slug" @click.prevent="searchRoute(dungeon)" class="subheader">{{ boss.text }}</a>

                  <a v-else-if="boss.slug" v-bind:key="boss.id" :href="'/dragonflight-mdt/' + boss.slug"
        ...
        <!-- <md-subheader>{{ $t("Dragonflight Season 3 Affix Weeks") }}</md-subheader>
        <md-list class="md-double-line md-dense">
          <md-list-item class="md-inset affixWeek">
            <category-image group="affixWeek"></category-image>
            <div class="md-list-text-container">
              <span>
                <a href="#" v-for="(item, index) in affixesS3" :key="index" @click.prevent="searchRoute(item.slug)">
        ...
        <!-- <md-subheader>{{ $t("Affixes") }}</md-subheader>
        <md-list class="md-double-line md-dense">
...
export default {
  ...
  methods: {
    ...
    searchRoute: function (item) {
      ...
      this.$router.push('/dragonflight-mdt/' + item.slug)
    },
    createMDT: function () {
      try {
  ...
  computed: {
    dungeons: function () {
      return categories.raidCategories(['df-mdt-s3'], 'MDT')
    },
    affixesS3: function () {
...
#search-mdt .md-list-item.md-inset .md-list-item-container {
  padding-left: 24px;
  flex-wrap: wrap;
  padding-bottom: 16px
}
