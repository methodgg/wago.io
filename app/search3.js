...
module.exports = function(criteria, req, res, searchCallback) {
    var results = []
    var sort = criteria.sort || false; delete criteria.sort
    if (sort=='Stars') {
        sort = { "popularity.favorite_count": -1 }
    }
    else if (sort=='Views') {
        sort = { "popularity.views": -1 }
    }
    else if (sort=='Comments') {
        sort = { "popularity.comments_count": -1 }
    }
    else if (sort=='Comment Date') {
        sort = { "last_comment": -1 }
    }
    else { // date
        sort = { "modified": -1 }
    ...
    if (criteria.searchProperties && criteria.searchProperties.searchUsers) {
        ...
        wagoCodeSearch(criteria, results, req, res, sort, page, max_results, 0, 0, mywago, searchCallback)
    }
    else {
        wagoSimpleSearch(criteria, results, req, res, sort, page, max_results, 0, 0, mywago, searchCallback)
    }
}

function wagoSimpleSearch(criteria, results, req, res, sort, page, max_results, skip, limit, mywago, searchCallback) {
    ...
    if (Object.keys(criteria).length>0)
        ...
        var search = { $and: [ privacyFilter, {deleted: false} ]}

    var total_results = 0

    async.parallel([
        // count total results of search
        function(parallel_cb) {
            Wago.count(search, function(err, count) {
                total_results = count
                parallel_cb(err, count)
            })
        },
        ...
        function(parallel_cb) {
            Wago.find(search).sort(sort).skip(14*page-skip).limit(14-limit).exec(function(err, foundResults) {
                    ...
                    async.parallel([
                        ...
                        function(parallel_cb2) {
                            Screenshots.findOne({auraID: results[async_key]._id}, function(err, screen) {
                                if (err || !screen) return parallel_cb2()
                        ...
                        // get username
                        function(parallel_cb2) {
                            if (!results[async_key]._userId) return parallel_cb2()

                            User.findById(results[async_key]._userId, function(err, user) {
                                if (err || !user) return parallel_cb2()
                ...
                function(err) { // forEachOf callback
                    parallel_cb(err, results) // search callback
                })
            })
        }],
        ...
        function(err, found) {
            var searchResults = {}
            searchResults.total = found[0]
            searchResults.results = found[1]
            searchCallback(err, searchResults)
        }
    )
}

function wagoCodeSearch(criteria, results, req, res, sort, page, max_results, skip, limit, mywago, callback) {
    delete criteria.searchProperties
