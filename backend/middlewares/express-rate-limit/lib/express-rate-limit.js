...
function RateLimit(options) {
  options = defaults(options, {
    ...
    expectUser: false, // do not count auth requests where user is found
    ifUser: function(req) { // update record with user id
      if (req._rateLimit && req.user) {
        req._rateLimit._userId = req.user._id
        req._rateLimit.save()
      }
    },
  ...
  function rateLimit(req, res, next) {
    ...
    options.store.incr(key, function(err, current, _rateLimit) {
      if (err) {
        return next(err);
      }

      req._rateLimit = _rateLimit
      ...
      Promise.resolve(maxResult)
        ...
        .then(max => {
          ...
          if (options.skipFailedRequests || options.skipSuccessfulRequests || options.expectUser) {
