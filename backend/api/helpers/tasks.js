...
async function UpdateTopLists() {
  ...
  imports = await WagoItem.find({ type: 'CATA-WEAKAURA', deleted: false, hidden: false, private: false, restricted: false, encrypted: false }).sort("-popularity.favorite_count").select('_id name popularity.favorite_count previewImage previewStatic').limit(10).exec()
  data.push({ title: 'Favorite Cataclysm WeakAuras All Time', imports: imports.map(x => { return { count: x.popularity.favorite_count, display: '[-count-] star', name: x.name, slug: x.slug, img: x.previewImage, static: x.previewStatic } }) })
  imports = await WagoItem.find({ type: 'MOP-WEAKAURA', deleted: false, hidden: false, private: false, restricted: false, encrypted: false }).sort("-popularity.favorite_count").select('_id name popularity.favorite_count previewImage previewStatic').limit(10).exec()
  data.push({ title: 'Favorite Mists of Pandaria Classic WeakAuras All Time', imports: imports.map(x => { return { count: x.popularity.favorite_count, display: '[-count-] star', name: x.name, slug: x.slug, img: x.previewImage, static: x.previewStatic } }) })
  ...
  imports = await WagoItem.find({ type: 'CATA-WEAKAURA', deleted: false, hidden: false, private: false, restricted: false, encrypted: false }).sort("-popularity.viewsThisWeek").select('_id name popularity.viewsThisWeek previewImage previewStatic').limit(10).exec()
  data.push({ title: 'Popular Cataclysm WeakAuras This Week', imports: imports.map(x => { return { count: x.popularity.viewsThisWeek, display: '[-count-] view', name: x.name, slug: x.slug, img: x.previewImage, static: x.previewStatic } }) })
  imports = await WagoItem.find({ type: 'MOP-WEAKAURA', deleted: false, hidden: false, private: false, restricted: false, encrypted: false }).sort("-popularity.viewsThisWeek").select('_id name popularity.viewsThisWeek previewImage previewStatic').limit(10).exec()
  data.push({ title: 'Popular Mists of Pandaria Classic WeakAuras This Week', imports: imports.map(x => { return { count: x.popularity.viewsThisWeek, display: '[-count-] view', name: x.name, slug: x.slug, img: x.previewImage, static: x.previewStatic } }) })
  ...
  imports = await WagoItem.find({ type: 'CATA-WEAKAURA', deleted: false, hidden: false, private: false, restricted: false, encrypted: false }).sort("-popularity.installed_count").select('_id name popularity.installed_count previewImage previewStatic').limit(10).exec()
  data.push({ title: 'Installed Cataclysm WeakAuras', imports: imports.map(x => { return { count: x.popularity.installed_count, display: '[-count-] install', name: x.name, slug: x.slug, img: x.previewImage, static: x.previewStatic } }), lastOfSection: true })
  imports = await WagoItem.find({ type: 'MOP-WEAKAURA', deleted: false, hidden: false, private: false, restricted: false, encrypted: false }).sort("-popularity.installed_count").select('_id name popularity.installed_count previewImage previewStatic').limit(10).exec()
  data.push({ title: 'Installed Mists of Pandaria Classic WeakAuras', imports: imports.map(x => { return { count: x.popularity.installed_count, display: '[-count-] install', name: x.name, slug: x.slug, img: x.previewImage, static: x.previewStatic } }), lastOfSection: true })
...
async function ProcessCode(data) {
  ...
  if (!doc || !code || !code._id || doc.encrypted || doc.type === 'OPIE') {
  ...
  cloudflare.zones.purgeCache(config.cloudflare.zoneID, {
    files: [
      ...
      { url: `https://data.wago.io/lookup/wago/code?id=${doc._id}&version=${code.versionString}&domain=${doc.domain}` },
      { url: `https://data.wago.io/lookup/wago/code?id=${doc.slug}&version=${code.versionString}&domain=${doc.domain}` },
      { url: `https://data.wago.io/lookup/wago/code?id=${doc._id}&version=${code.versionString}-${doc.latestVersion.iteration}&domain=${doc.domain}` },
      { url: `https://data.wago.io/lookup/wago/code?id=${doc.slug}&version=${code.versionString}-${doc.latestVersion.iteration}&domain=${doc.domain}` },
