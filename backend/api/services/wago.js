...
server.get('/wago/embed', (req, res, next) => {
  ...
  WagoItem.lookup(req.params.id).then((wago) => {
    ...
    WagoCode.lookup(wago._id).then((code) => {
      ...
      js = js + `document.write('<span id="wago-${embed.id}" class="wagoEmbed"><a href="${embed.url}"><img src="${embed.theme.logo}"></a><button onclick="wagoCopy(this, wago.c${wago.id})" class="wagoCopyButton"><small class="clickToCopyWago">Click to copy import string from wago.io</small><div class="wagoName">${embed.name}</div></button></span>');`  
